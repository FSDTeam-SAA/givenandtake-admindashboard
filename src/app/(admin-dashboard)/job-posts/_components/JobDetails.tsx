...
const updateJobStatus = async ({ id, status }: { id: string; status: string }) => {
  const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/jobs/${id}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ jobApprove: status }),
  })
  if (!response.ok) {
...
// Utility function to parse tags (e.g., #tag) into styled elements
const parseDescriptionWithTags = (description: string) => {
  const tagRegex = /#(\w+)/g
  return description.split(tagRegex).map((part, index) => {
    if (index % 2 === 1) {
      // This is a tag (captured group from regex)
      return (
        <span
          key={index}
          className="inline-block bg-blue-100 text-blue-800 text-sm font-medium px-2 py-0.5 rounded mx-1"
        >
          #{part}
        </span>
      )
...
export default function JobDetails({ jobId, onBack }: JobDetailsProps) {
  ...
  // Use a consistent timezone for date formatting to avoid hydration mismatch
  const formatDate = (dateString: string) => {
    const date = new Date(dateString)
    return date.toLocaleDateString("en-US", {
      day: "numeric",
  ...
  // Prevent rendering until the client is ready to avoid hydration mismatch
  if (typeof window === "undefined") {
    return null // Skip rendering on the server to match client
  }
      ...
      <div className="max-w-6xl mx-auto overflow-hidden">
        ...
        <div className="bg-[#DFFAFF] p-4 md:p-6 flex items-center gap-4">
          <Button
            ...
            size="icon"
            onClick={onBack}
            className="text-[#44B6CA] hover:bg-[#44B6CA]/10"
          >
            <ChevronLeft className="!h-[32px] !w-[32px]" />
            <span className="sr-only">Back</span>
          </Button>
          <h1 className="text-2xl md:text-[40px] font-bold text-[#44B6CA]">
            Job Post Details
          </h1>
        </div>
 ...
 Deadline:
        ...
        <div className="p-4 md:p-6">
          ...
          <div className="mb-6">
            ...
            <div className="text-base text-[#707070] leading-relaxed font-medium">
              {parseDescriptionWithTags(job.description)}
            </div>
          </div>
          ...
          <div className="flex justify-end gap-4 mt-8">
            <Button
              variant="outline"
              className="px-6 py-2 text-[#595959] border-[#BFBFBF] hover:bg-gray-100 bg-transparent"
              onClick={() => mutation.mutate({ id: jobId, status: "denied" })}
            ...
            <Button
              ...
              onClick={() => mutation.mutate({ id: jobId, status: "approved" })}
              disabled={mutation.isPending}
            >
              Approve
            </Button>
