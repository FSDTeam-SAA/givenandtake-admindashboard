...
const deleteBlog = async ({ blogId, token }: { blogId: string; token: string }): Promise<void> => {
  const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/blogs/${blogId}`, {
    method: "DELETE",
    headers: {
      "Authorization": `Bearer ${token}`,
    },
...
export default function Component() {
  ...
  if (error) {
    return <div className="min-h-screen bg-gray-50 flex items-center justify-center">Error: {error.message}</div>
  }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[15px]">
              {isLoading ? (
                Array(6).fill(0).map((_, index) => (
                  <Card key={index} className="overflow-hidden border-none shadow-none p-0 mt-10">
                ...
                blogPosts?.map((post) => (
                  <Card key={post._id} className="overflow-hidden border-none shadow-none p-0 mt-10">
                    <div className="w-full">
                      <Image
                      ...
                      {post.tags && post.tags.length > 0 && (
                        <div className="flex flex-wrap gap-2 mt-2">
                          {post.tags.map((tag, index) => (
                            <span
                              key={index}
                              className="px-2 py-1 bg-[#9EC7DC]/20 text-[#44B6CA] text-xs rounded-full"
                            >
                              {tag}
                            </span>
                          ))}
